...
public class Util {
	...
	public static String getDuration() {
		Duration duration = Duration.ofNanos(getElapsedTime());
		StringBuilder sb = new StringBuilder();
		if (duration.toDays() > 0)
			sb.append(String.format("%02 Day ", duration.toDays()));
		if (duration.toHours() > 0)
			sb.append(String.format("%02 Hour ", duration.toHours() % 24));
		if (duration.toMinutes() > 0)
			sb.append(String.format("%02 Minutes ", duration.toMinutes() % 60));
		if (duration.toSeconds() > 0)
			sb.append(String.format("%02 Seconds ", duration.toSeconds() % 60));
		if (duration.toMillis() > 0)
			sb.append(String.format("%02 Millisecond ", duration.toMillis() % 1000));
		if (duration.toNanos() > 0)
			sb.append(String.format("%02 Nanosecond ", duration.toNanos() % 1000000L));
